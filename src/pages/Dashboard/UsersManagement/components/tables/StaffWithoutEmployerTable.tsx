...
const StaffWithoutEmployerTable = ({ data, currentPage }) => {
  const { navigateToStaffProfile } = useNavigateTo();
  const handleOnclick = (id) => {
    navigateToStaffProfile(id);
  };

  // Filter out staff members where employerId is null
  const filteredData = data.filter(staff => staff?.user?.employerId === null);
...
export default StaffWithoutEmployerTable;
